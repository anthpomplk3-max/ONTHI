...
def load_document()
    document_content = 
    [Nội dung tài liệu QT Hóa Sinh sẽ được đọc ở đây]
    
    
    # Chia tài liệu thành các phần
    sections = document_content.split('###')
    
    # Tách thành các đoạn nhỏ hơn
    paragraphs = []
    for section in sections
        if section.strip()
            # Tách theo dòng và lọc các dòng trống
            lines = [line.strip() for line in section.split('n') if line.strip()]
...
def calculate_statistics(text)
    ...
    # Tính thời gian đọc ước tính (từphút)
    reading_time_minutes = word_count  150  # 150 từphút
    
    # Tần suất từ
    word_freq = Counter(words.lower() for words in words)
...
# Giao diện chính
def main()
    st.markdown(h1 class='main-header'🔬 QT Hóa Sinh - Trình Đọc Tài Liệuh1, unsafe_allow_html=True)
    ...
    with st.sidebar
        st.markdown(h3 class='sidebar-header'🎛️ Điều Khiển Đọch3, unsafe_allow_html=True)
        ...
        st.markdown(h3 class='sidebar-header'⚙️ Cài Đặth3, unsafe_allow_html=True)
        ...
        # Thống kê
        st.markdown(h3 class='sidebar-header'📊 Thống Kê Tài Liệuh3, unsafe_allow_html=True)
        ...
        st.markdown(f
        div class='stats-box'
            b📄 Tổng số đoạnb {stats['paragraph_count']}br
            b🔤 Tổng số từb {stats['word_count']}br
            b📝 Tổng số câub {stats['sentence_count']}br
            b⏱️ Thời gian đọc ước tínhb {stats['reading_time'].1f} phútbr
        div
        , unsafe_allow_html=True)
    ...
    st.markdown(h3 class='sidebar-header'📖 Nội Dung Tài Liệuh3, unsafe_allow_html=True)
    ...
    progress_text = fĐang đọc Đoạn {st.session_state.current_index + 1}{len(paragraphs)}
    progress = (st.session_state.current_index + 1)  len(paragraphs)
    ...
    div style='text-align center; color #666;'
        pQT Hóa Sinh - Trình đọc tài liệu  Sử dụng pyttsx3 & Streamlitp
        pChức năng Đọc toàn bộ tài liệu • Highlight phần đang đọc • Điều chỉnh âm lượngtốc độ • Thống kêp
    div
    , unsafe_allow_html=True)

# Tải nội dung tài liệu từ biến trong code
def load_document()
    # Tài liệu QT Hóa Sinh
    document_content = 
    ...
    ## NHẬN ĐỊNH KẾT QUẢ
    - Giá trị tham chiếu  200 IU10^12 Hồng cầu
    ...
    # Chia thành các đoạn
    paragraphs = []
    for line in document_content.strip().split('n')
        if line.strip()
